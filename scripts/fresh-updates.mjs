...
async function fetchEdition(edition) { const url = `https://news.google.com/rss/search?q=${encodeURIComponent(`${edition.query} when:2d`)}&hl=en-IN&gl=IN&ceid=IN:en`; const response = await fetch(url, { headers: { 'user-agent': 'BiggBossIndiaFreshUpdater/3.0' } }); if (!response.ok) throw new Error(`${edition.label}: RSS HTTP ${response.status}`); return { ...edition, items: extractItems(await response.text()).slice(0, 6) } }

// Turn a headline/feed snippet into an original viewer-style editorial read.
// Source links are deliberately kept out of the rendered article: sources inform
// the analysis engine, but the page itself reads like an independent daily column.
function analyze(edition, item) {
  ...
  let watch = 'I would watch the next episode closely for the small reactions rather than only the loudest moment. That is usually where alliances, insecurity and individual strategy become visible.'

  if (/evict|eliminat|out of the house/.test(text)) {
    opening = `This is the kind of early-season twist that can quietly change everything. ${subject} means one seat has suddenly disappeared, and the remaining housemates now have to adjust their numbers, relationships and expectations.`
    game = 'The interesting question is who benefits from the exit. Someone who was depending on that relationship may become exposed, while a rival may suddenly get more room to control the group. Early eliminations can also force contestants to stop playing socially safe and start showing their real priorities.'
    watch = 'I would watch who talks about the exit first, who stays unusually quiet, and which contestants immediately start rebuilding their group. Those reactions can tell us more about the next nomination battle than the eviction itself.'
  } else if (/wild.?card|entrant|enters|joins/.test(text)) {
    opening = `This arrival comes at exactly the point where the original house structure is beginning to form. ${subject} gives the game a new variable when existing contestants are already starting to understand one another.`
    game = 'A new contestant has one major advantage: they can observe before committing. If the entrant avoids becoming somebody else’s weapon and instead builds independent relationships, the established groups could suddenly become much less comfortable.'
    watch = 'The first few conversations matter more than the first argument. I would watch who welcomes the newcomer, who tries to recruit them immediately, and who looks threatened by their presence.'
  } else if (/captain|captaincy/.test(text)) {
    opening = `Captaincy is where the house starts revealing its power structure. ${subject} matters because the title is not only about authority; it exposes who can gather support when the game becomes competitive.`
    game = 'The next test is whether the captain can actually lead without creating a larger opposition group. A contestant who uses power too aggressively may gain short-term control but create nominations waiting for them later.'
    watch = 'I would watch the captain’s first difficult decision, who gets protected, who feels ignored and which contestants start positioning themselves for the next power shift.'
  } else if (/nominat|voting|vote/.test(text)) {
    opening = `This nomination development gives us the first real picture of where pressure is building. ${subject} matters because nominations turn casual friendships into strategic calculations.`
    game = 'Once names are on the table, every contestant has to decide whether to protect relationships or protect their own game. That is where quiet alliances often become visible, especially when several people suddenly have the same target.'
    watch = 'I would watch for repeated names, unexpected votes and contestants who suddenly become very active in conversations. The pattern is often more revealing than the final list.'
  } else if (/fight|clash|controvers|argument|conflict|heated/.test(text)) {
    opening = `The argument itself is only the surface of this story. ${subject} is interesting because repeated friction can reveal which personalities are already struggling to coexist.`
    game = 'If the disagreement stays personal, it may burn out quickly. If it becomes connected to tasks, food, leadership or nominations, it can turn into a proper season storyline and pull other contestants into sides.'
    watch = 'I would watch who keeps returning to the issue after the cameras move on from the obvious confrontation. The second reaction is often more important than the first shout.'
  } else if (/task|challenge|game/.test(text)) {
    opening = `This task is more important than the result alone suggests. ${subject} puts contestants under pressure, and pressure is where natural leadership, teamwork and selfishness start becoming visible.`
    game = 'A strong performance can create credibility, but a poor performance can also become ammunition in the next nomination discussion. The contestants who understand that balance usually survive longer than those who chase one dramatic moment.'
    watch = 'I would watch who performs for the team, who performs only for themselves and who tries to control the room without actually delivering in the task.'
  } else if (/cyber|harass|mental health|personal struggle|suicid/.test(text)) {
    opening = `This is a very different kind of Bigg Boss moment because the story goes beyond the competition. ${subject} brings a personal experience into the house, and that can change how viewers understand the contestant.`
    game = 'It should not be reduced to a gameplay angle. At the same time, moments like this can change relationships inside the house because vulnerability often creates trust, concern or sometimes uncomfortable distance.'
    watch = 'I would watch the housemates’ response rather than turn a personal disclosure into a prediction. Compassion, support and respect matter more here than creating unnecessary drama.'
  }
...
const dashboard = results.map(edition => {
  ...
  if (!item) return `<section class="status"><h2>${escapeHtml(edition.label)}</h2><p>No fresh item was available in this refresh window. The edition tracker remains live.</p><p><a href="${edition.href}">Open edition tracker →</a></p></section>`
  const analysis = analyze(edition, item)
...
await writeFile('.fresh-update-state.json', JSON.stringify({ updatedAt: now.toISOString(), date, editions: editionJson, itemCount: latest.length, editorialAnalysis: true, externalLinksDisplayed: false }, null, 2) + '\n')
console.log(`Published ${latest.length} fresh Bigg Boss developments with deep editorial analysis across ${results.length} editions for ${displayDate}.`)
