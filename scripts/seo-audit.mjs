...
for (const url of urls) {
  ...
  const localPath = pathname === '/' ? 'index.html' : `${pathname.replace(/^\//, '').replace(/\/$/, '')}/index.html`
  let html
  try {
  ...
  if (title.length < 20 || title.length > 80) fail(`${localPath}: title length should be 20-80 characters`)
...
console.log(`Bigg Boss India SEO audit passed for ${urls.length} indexable pages.`)
